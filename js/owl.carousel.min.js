! function(a, b, c, d) {
    ...
    l = {
        ...
        if (this.trigger("initialize"), this.dom.$el.hasClass(this.options.baseClass) || this.dom.$el.addClass(this.options.baseClass), this.dom.$el.hasClass(this.options.themeClass) || this.dom.$el.addClass(this.options.themeClass), this.options.rtl && this.dom.$el.addClass("owl-rtl"), this.browserSupport(), this.sortOptions(), this.setResponsiveOptions(), this.options.autoWidth && this.state.imagesLoaded !== !0) {
            var a, b, c;
            if (a = this.dom.$el.find("img"), b = this.options.nestedItemSelector ? "." + this.options.nestedItemSelector : d, c = this.dom.$el.children(b).width(), a.length && 0 >= c) return this.preloadAutoWidthImages(a), !1
        }
        ...
        var a, b, c, d, e = [];
        a = this.options.responsive, this.responsiveSorted = {};
        for (b in a) e.push(b);
        ...
        var a, b, c, d, e;
        a = this.windowWidth(), resOpt = this.options.responsive;
        for (d in this._options) "responsive" !== d && (this.options[d] = this._options[d]);
        for (b in this.responsiveSorted)
            if (a >= b) {
                e = b;
                for (c in this.responsiveSorted[e]) this.options[c] = this.responsiveSorted[e][c]
            }
        this.num.breakpoint = e, this.options.responsiveClass && this.dom.$el.attr("class", function(a, b) {
        ...
        var b = c.createElement(this.options.itemElement);
        return b.className = this.options.itemClass, a(b)
        ...
        var b, c, d, e = this.options.items,
            f = this.num.oItems - 1;
        for (this.options.stagePadding && 1 === this.options.items && (e += 1), this.num.cItems = 2 * e, d = 0; e > d; d++) b = this.cloneItemContainer(this.dom.$oItems.eq(d)), c = this.cloneItemContainer(this.dom.$oItems.eq(f - d)), this.dom.$stage.append(b), this.dom.$stage.prepend(c);
        ...
        var a, b, c, d, e, f, g, h = 0,
            i = 0;
        for (this.width.el = this.dom.$el.width() - 2 * this.options.stagePadding, this.width.view = this.dom.$el.width(), c = this.width.el - this.options.margin * (1 === this.options.items ? 0 : this.options.items - 1), this.width.el = this.width.el + this.options.margin, this.width.item = (c / this.options.items + this.options.margin).toFixed(3), this.dom.$items = this.dom.$stage.find(".owl-item"), this.num.items = this.dom.$items.length, this.options.autoWidth && this.dom.$items.css("width", ""), this.pos.items = [], this.num.merged = [], d = this.options.rtl ? this.options.center ? -(this.width.el / 2) : 0 : this.options.center ? this.width.el / 2 : 0, this.width.mergeStage = 0, a = 0; a < this.num.items; a++) this.options.merge ? (g = this.dom.$items.eq(a).find("[data-merge]").attr("data-merge") || 1, this.options.mergeFit && g > this.options.items && (g = this.options.items), this.num.merged.push(parseInt(g)), this.width.mergeStage += this.width.item * this.num.merged[a]) : this.num.merged.push(1), f = this.width.item * this.num.merged[a], this.options.autoWidth && (f = this.dom.$items.eq(a).width() + this.options.margin, this.options.rtl ? this.dom.$items[a].style.marginLeft = this.options.margin + "px" : this.dom.$items[a].style.marginRight = this.options.margin + "px"), this.pos.items.push(d), this.dom.$items.eq(a).data("owl-item").posLeft = h, this.dom.$items.eq(a).data("owl-item").width = f, this.options.rtl ? (d += f, h += f) : (d -= f, h -= f), i -= Math.abs(f), this.options.center && (this.pos.items[a] = this.options.rtl ? this.pos.items[a] + f / 2 : this.pos.items[a] - f / 2);
        for (this.width.stage = Math.abs(this.options.autoWidth ? this.options.center ? i : d : i), e = this.num.oItems + this.num.cItems, b = 0; e > b; b++) this.dom.$items.eq(b).data("owl-item").indexAbs = b;
        this.setMinMax(), this.setSizes()
    }, e.prototype.setMinMax = function() {
        var a, b, c;
        if (b = this.dom.$oItems.eq(0).data("owl-item").indexAbs, this.pos.min = 0, this.pos.minValue = this.pos.items[b], this.options.loop || (this.pos.max = this.num.oItems - 1), this.options.loop && (this.pos.max = this.num.oItems + this.options.items), this.options.loop || this.options.center || (this.pos.max = this.num.oItems - this.options.items), this.options.loop && this.options.center && (this.pos.max = this.num.oItems + this.options.items), this.pos.maxValue = this.pos.items[this.pos.max], !this.options.loop && !this.options.center && this.options.autoWidth || this.options.merge && !this.options.center) {
            for (c = this.options.rtl ? 1 : -1, a = 0; a < this.pos.items.length; a++) this.pos.items[a] * c < this.width.stage - this.width.el && (this.pos.max = a + 1);
        ...
        this.options.stagePadding !== !1 && (this.dom.oStage.style.paddingLeft = this.options.stagePadding + "px", this.dom.oStage.style.paddingRight = this.options.stagePadding + "px"), this.options.rtl ? b.setTimeout(a.proxy(function() {
            this.dom.stage.style.width = this.width.stage + "px"
        }, this), 0) : this.dom.stage.style.width = this.width.stage + "px";
        for (var c = 0; c < this.num.items; c++) this.options.autoWidth || (this.dom.$items[c].style.width = this.width.item - this.options.margin + "px"), this.options.rtl ? this.dom.$items[c].style.marginLeft = this.options.margin + "px" : this.dom.$items[c].style.marginRight = this.options.margin + "px", 1 === this.num.merged[c] || this.options.autoWidth || (this.dom.$items[c].style.width = this.width.item * this.num.merged[c] - this.options.margin + "px");
        ...
        return this.trigger("refresh"), this.setResponsiveOptions(), this.updateLocalContent(), this.optionsLogic(), 0 === this.num.oItems ? !1 : (this.dom.$stage.addClass("owl-refresh"), this.reClone(), this.calculate(), this.dom.$stage.removeClass("owl-refresh"), a ? this.initPosition() : this.jumpTo(this.pos.current, !1), this.state.orientation = b.orientation, this.watchVisibility(), void this.trigger("refreshed"))
        ...
        this.dragType = a && !d ? ["touchstart", "touchmove", "touchend", "touchcancel"] : a && d ? ["MSPointerDown", "MSPointerMove", "MSPointerUp", "MSPointerCancel"] : ["mousedown", "mousemove", "mouseup"], (a || d) && this.options.touchDrag ? this.on(c, this.dragType[3], this.e._onDragEnd) : (this.dom.$stage.on("dragstart", function() {
            ...
            return !1
        } : this.dom.$el.addClass("owl-text-select-on")), this.transitionEndVendor && this.on(this.dom.stage, this.transitionEndVendor, this.e._transitionEnd, !1), this.options.responsive !== !1 && this.on(b, "resize", this.e._resizer, !1), this.dragEvents()
        ...
        var c, e, f, g, h, i, j;
        this.state.isTouch && (this.state.isScrolling || (c = a.originalEvent || a || b.event, e = "touchmove" == c.type, f = e ? c.targetTouches[0].pageX : c.pageX || c.clientX, g = e ? c.targetTouches[0].pageY : c.pageY || c.clientY, this.drag.currentX = f - this.drag.startX, this.drag.currentY = g - this.drag.startY, this.drag.distance = this.drag.currentX - this.drag.offsetX, this.drag.distance < 0 ? this.state.direction = this.options.rtl ? "right" : "left" : this.drag.distance > 0 && (this.state.direction = this.options.rtl ? "left" : "right"), this.options.loop ? this.op(this.drag.currentX, ">", this.pos.minValue) && "right" === this.state.direction ? this.drag.currentX -= this.pos.loop : this.op(this.drag.currentX, "<", this.pos.maxValue) && "left" === this.state.direction && (this.drag.currentX += this.pos.loop) : (h = this.options.rtl ? this.pos.maxValue : this.pos.minValue, i = this.options.rtl ? this.pos.minValue : this.pos.maxValue, j = this.options.pullDrag ? this.drag.distance / 5 : 0, this.drag.currentX = Math.max(Math.min(this.drag.currentX, h + j), i + j)), (this.drag.distance > 8 || this.drag.distance < -8) && (c.preventDefault !== d ? c.preventDefault() : c.returnValue = !1, this.state.isSwiping = !0), this.drag.updatedX = this.drag.currentX, (this.drag.currentY > 16 || this.drag.currentY < -16) && this.state.isSwiping === !1 && (this.state.isScrolling = !0, this.drag.updatedX = this.drag.start), this.animStage(this.drag.updatedX)))
        ...
        if (!this.options.freeDrag)
            for (b = 0; b < this.num.items; b++) a > this.pos.items[b] - d && a < this.pos.items[b] + d ? c = b : this.op(a, "<", this.pos.items[b]) && this.op(a, ">", this.pos.items[b + 1 || this.pos.items[b] - this.width.el]) && (c = "left" === this.state.direction ? b + 1 : b);
        return this.options.loop || (this.op(a, ">", this.pos.minValue) ? c = a = this.pos.min : this.op(a, "<", this.pos.maxValue) && (c = a = this.pos.max)), this.options.freeDrag ? a : (this.pos.currentAbs = c, this.pos.current = this.dom.$items.eq(c).data("owl-item").index, c)
    }, e.prototype.animStage = function(b, c) {
        b !== this.pos.stage && c !== !0 && (this.trigger("translate"), this.state.inMotion = !0);
        var d = this.pos.stage = b,
            e = this.dom.stage.style;
        this.support3d ? (translate = "translate3d(" + d + "px,0px, 0px)", e[this.transformVendor] = translate) : this.state.isTouch ? e.left = d + "px" : this.dom.$stage.animate({
            left: d
        }, this.speed.css2speed, this.options.fallbackEasing, a.proxy(function() {
            this.state.inMotion && this.transitionEnd()
        }, this)), this.onChange()
    }, e.prototype.updatePosition = function(a) {
        if (0 === this.num.oItems || a === d) return !1;
        this.pos.prev = this.pos.currentAbs, this.state.revert || this.options.loop ? this.state.revert || (a = a >= this.num.oItems ? this.num.oItems - 1 : a) : a = a > this.pos.max ? this.pos.max : 0 >= a ? 0 : a;
        ...
        a = b.data ? b.data : a, this.state.revert ? (this.pos.current = this.dom.$items.eq(a).data("owl-item").index, this.pos.currentAbs = a) : (this.pos.current = this.dom.$oItems.eq(a).data("owl-item").index, this.pos.currentAbs = this.dom.$oItems.eq(a).data("owl-item").indexAbs), this.trigger("changed", {
            property: {
                name: "position",
                value: a
            }
        ...
        var e, f, g = a,
            h = b;
        return (g === !1 && 0 !== g && c !== !0 || g === d) && (e = Math.abs(h - this.pos.prev), e = 0 === e ? 1 : e, e > 6 && (e = 6), g = e * this.options.smartSpeed), g === !1 && c === !0 && (g = this.options.smartSpeed), 0 === g && (g = 0), this.support3d ? (f = this.dom.stage.style, f.webkitTransitionDuration = f.MsTransitionDuration = f.msTransitionDuration = f.MozTransitionDuration = f.OTransitionDuration = f.transitionDuration = g / 1e3 + "s") : this.speed.css2speed = g, this.speed.current = g, g
    }, e.prototype.jumpTo = function(a, b) {
        this.updatePosition(a), this.setSpeed(0), this.animStage(this.pos.items[this.pos.currentAbs], b)
    }, e.prototype.to = function(c, d) {
        if (this.options.loop) {
            var e = c - this.pos.current,
                f = this.pos.currentAbs,
                g = this.pos.currentAbs,
                h = this.pos.currentAbs + e,
                i = 0 > g - h ? !0 : !1;
            this.state.revert = !0, h < this.options.items && i === !1 ? (this.state.bypass = !0, f = this.num.items - (this.options.items - g) - this.options.items, this.jumpTo(f, !0)) : h >= this.num.items - this.options.items && i === !0 && (this.state.bypass = !0, f = g - this.num.oItems, this.jumpTo(f, !0)), b.clearTimeout(this.e._goToLoop), this.e._goToLoop = b.setTimeout(a.proxy(function() {
                this.state.bypass = !1, this.updatePosition(f + e), this.setSpeed(d, this.pos.currentAbs), this.animStage(this.pos.items[this.pos.currentAbs]), this.state.revert = !1
            }, this), 30)
        } else this.updatePosition(c), this.setSpeed(d, this.pos.currentAbs), this.animStage(this.pos.items[this.pos.currentAbs])
    }, e.prototype.next = function(a) {
        a = a || !1, this.to(this.pos.current + 1, a)
    }, e.prototype.prev = function(a) {
        a = a || !1, this.to(this.pos.current - 1, a)
    }, e.prototype.initPosition = function() {
        if (!this.dom.$oItems) return !1;
        var a = this.options.startPosition,
        ...
        a = b.data || a, "number" == typeof a && this.options.center || (a = 0), this.dom.oStage.scrollLeft = 0, this.jumpTo(a, !0), this.trigger("changed", {
            property: {
                name: "position",
                value: a
            }
        ...
        b = b || 0, this.initializeItemContainer(c, a), 0 === this.dom.$oItems.length ? this.dom.$stage.append(c) : -1 !== q ? this.dom.$oItems.eq(b).before(c) : this.dom.$oItems.eq(b).after(c), this.refresh()
        ...
        var e = {
            ...
            g = a.Event([b, "owl", d || "carousel"].join(".").toLowerCase(), a.extend(e, c));
        return a.each(this.plugins, function(a, b) {
            b.onTrigger && b.onTrigger(g)
        }), this.suppressedEvents[g.type] || this.dom.$el.trigger(g), "function" == typeof this.options[f] && this.options[f].apply(this, g), g
...
function(a, b) {
    LazyLoad = function(b) {
        this.owl = b, this.owl.options = a.extend({}, LazyLoad.Defaults, this.owl.options), this.owl.options.lazyLoad && (this.handlers = {
...
function(a, b) {
    AutoHeight = function(b) {
        ...
        var a, c = this.owl.dom.$items.eq(this.owl.pos.currentAbs),
            d = this.owl.dom.$oStage,
            e = 0;
        this.owl.dom.$oStage.hasClass(this.owl.options.autoHeightClass) || this.owl.dom.$oStage.addClass(this.owl.options.autoHeightClass), a = b.setInterval(function() {
    ...
    Video = function(b) {
        this.owl = b, this.owl.options = a.extend({}, Video.Defaults, this.owl.options), this.handlers = {
            ...
            "refresh.owl.carousel refreshed.owl.carousel": a.proxy(function(a) {
                return this.owl.options.video ? void(this.refreshing = "refresh" == a.type) : !1
            }, this),
            "changed.owl.carousel": a.proxy(function(a) {
        ...
        var c, d, e, f, g = a.data("vimeo-id"),
            h = a.data("youtube-id"),
            i = a.data("width") || this.owl.options.videoWidth,
            j = a.data("height") || this.owl.options.videoHeight,
            k = a.attr("href");
        if (g) d = "vimeo", e = g;
        ...
        function d(a) {
            f = '<div class="owl-video-play-icon"></div>', e = k.options.lazyLoad ? '<div class="owl-video-tn ' + j + '" ' + i + '="' + a + '"></div>' : '<div class="owl-video-tn" style="opacity:1;background-image:url(' + a + ')"></div>', b.after(e), b.after(f)
        }
        var e, f, g, h = b.find("img"),
            i = "src",
            j = "",
            k = this.owl;
        return this.owl.options.lazyLoad && (i = "data-src", j = "owl-lazy"), h.length ? (d(h.attr(i)), h.remove(), !1) : void("youtube" === c.type ? (g = "http://img.youtube.com/vi/" + c.id + "/hqdefault.jpg", d(g)) : "vimeo" === c.type && a.ajax({
            type: "GET",
            url: "http://vimeo.com/api/v2/video/" + c.id + ".json",
            jsonp: "callback",
            dataType: "jsonp",
            success: function(a) {
                g = a[0].thumbnail_large, d(g), k.options.loop && k.updateActiveItems()
            }
        }))
        ...
        var c, d, e, f = a(b.target || b.srcElement),
            g = f.closest("." + this.owl.options.itemClass);
...
function(a) {
    Animate = function(b) {
        this.owl = b, this.owl.options = a.extend({}, Animate.Defaults, this.owl.options), this.swapping = !0, (this.owl.options.animateIn || this.owl.options.animateOut) && (this.handlers = {
        ...
        this.owl.setSpeed(0);
        var b, c, d, e, f = this.owl.dom.$items.eq(this.owl.pos.prev),
            g = Math.abs(f.data("owl-item").width) * this.owl.pos.prev,
            h = this.owl.dom.$items.eq(this.owl.pos.currentAbs),
            i = Math.abs(h.data("owl-item").width) * this.owl.pos.currentAbs;
        return this.owl.pos.currentAbs === this.owl.pos.prev ? !1 : (b = i - g, c = this.owl.options.animateIn, d = this.owl.options.animateOut, e = this.owl, removeStyles = function() {
            a(this).css({
                left: ""
            }).removeClass("animated owl-animated-out owl-animated-in").removeClass(c).removeClass(d), e.transitionEnd()
        }, d && f.css({
            left: b + "px"
        }).addClass("animated owl-animated-out " + d).one("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend", removeStyles), void(c && h.addClass("animated owl-animated-in " + c).one("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend", removeStyles)))
...
function(a, b, c) {
    Autoplay = function(b) {
        this.owl = b, this.owl.options = a.extend({}, Autoplay.Defaults, this.owl.options), this.handlers = {
        ...
        this.owl.options.autoplay && !this.owl.state.videoPlay ? (b.clearInterval(this.apInterval), this.apInterval = b.setInterval(a.proxy(function() {
            this.play()
        }, this), this.owl.options.autoplayTimeout)) : (b.clearInterval(this.apInterval), this.autoplayState = !1)
...
function(a) {
    "use strict";
    var b = function(c) {
        return this.core = c, this.core.options = a.extend({}, b.Defaults, this.core.options), this.refreshing = !1, this.initialized = !1, this.page = null, this.pages = [], this.controls = {}, this.template = null, this.$element = this.core.dom.$el, this.core.options.nav || this.core.options.dots ? (this.handlers = {
            "initialized.owl.carousel": a.proxy(function() {
                this.initialized || this.initialize()
            }, this),
            "changed.owl.carousel": a.proxy(function(b) {
                "items" == b.property.name && this.initialized && this.update(), this.filling && (b.property.value.data("owl-item").dot = a(":first-child", b.property.value).find("[data-dot]").andSelf().data("dot"))
            }, this),
            "change.owl.carousel": a.proxy(function(a) {
                if ("position" == a.property.name && !this.core.state.revert && !this.core.options.loop && this.core.options.navRewind) {
                    var b = this.core.pos;
                    a.data = a.property.value > b.max ? b.current >= b.max ? b.min : b.max : a.property.value < 0 ? b.max : a.property.value
                }
                this.filling = this.core.options.dotsData && "item" == a.property.name && a.property.value && a.property.value.is(":empty")
            }, this),
            "refresh.owl.carousel refreshed.owl.carousel": a.proxy(function(a) {
                this.refreshing = "refresh" == a.type
            }, this),
            "refreshed.owl.carousel": a.proxy(function() {
                this.initialized && this.refresh()
            }, this)
        }, void this.$element.on(this.handlers)) : !1
    };
    b.Defaults = {
        nav: !1,
        navRewind: !0,
        ...
        var b, c = this.core.options;
        this.refresh(), c.dotsData || (this.template = a("<div>").addClass(c.dotClass).append(a("<span>")).prop("outerHTML")), c.navContainer && c.dotsContainer || (this.controls.$container = a("<div>").addClass(c.controlsClass).appendTo(this.$element)), c.dots && (this.$indicators = c.dotsContainer ? a(c.dotsContainer) : a("<div>").addClass(c.dotsClass).appendTo(this.controls.$container), this.$indicators.on(this.core.dragType[2], "div", a.proxy(function(b) {
            var d = a(b.target).parent().is(this.$indicators) ? a(b.target).index() : a(b.target).parent().index();
            b.preventDefault(), this.core.to(this.pages[d].start, c.dotsSpeed)
        }, this))), c.nav && (b = c.navContainer ? a(c.navContainer) : a("<div>").addClass(c.navContainerClass).prependTo(this.controls.$container), this.controls.$next = a("<" + c.navElement + ">"), this.controls.$previous = this.controls.$next.clone(), this.controls.$previous.addClass(c.navClass[0]).text(c.navText[0]).prependTo(b).on(this.core.dragType[2], a.proxy(function() {
            this.core.to(this.core.pos.current - c.slideBy)
        }, this)), this.controls.$next.addClass(c.navClass[1]).text(c.navText[1]).appendTo(b).on(this.core.dragType[2], a.proxy(function() {
            this.core.to(this.core.pos.current + c.slideBy)
        ...
        var a, b, c, d = this.core.options,
            e = this.core.num.cItems / 2,
            f = this.core.num.items - e,
            g = this.core.num.oItems,
            h = d.center || d.autoWidth || d.dotData ? 1 : d.dotsEach || d.items;
        if (d.nav && (d.navRewind = g > d.items || d.center, d.slideBy = d.slideBy && "page" === d.slideBy ? d.items : Math.min(d.slideBy, d.items)), d.dots)
            for (this.pages = [], a = e, b = 0, c = 0; f > a; a++)(b >= h || 0 === b) && (this.pages.push({
                start: a - e,
                end: a - e + h - 1
        ...
        var b, c, d = "",
            ...
            g = this.core.pos.current;
        if (!e.nav || e.loop || e.navRewind || (this.controls.$previous.toggleClass("disabled", 0 >= g), this.controls.$next.toggleClass("disabled", g >= this.core.pos.max)), e.dots) {
            if (b = this.pages.length - this.$indicators.children().length, this.page = a.grep(this.pages, function(a) {
                    return a.start <= g && a.end >= g
                }).pop(), b > 0) {
                for (c = 0; c < Math.abs(b); c++) d += e.dotData ? f.eq(c).data("owl-item").dot : this.template;
                this.$indicators.append(d)
            } else 0 > b && this.$indicators.children().slice(b).remove();
            this.$indicators.find(".active").removeClass("active"), this.$indicators.children().eq(this.pages.indexOf(this.page) % f.length).addClass("active")
        }
    }, b.prototype.onTrigger = function(a) {
        var b = this.core.options;
        a.page = {
            index: this.pages.indexOf(this.page),
            count: this.pages.length,
            size: b.center || b.autoWidth || b.dotData ? 1 : b.dotsEach || b.items
        }
...
function(a, b) {
    "use strict";
    var c = function(d) {
        return this.carousel = d, this.options = a.extend({}, c.Defaults, this.carousel.options), this.hashes = {}, this.$element = this.carousel.dom.$el, this.options.URLhashListener ? (this.handlers = {
            "initialized.owl.carousel": a.proxy(function() {
            ...
            "change.owl.carousel": a.proxy(function(a) {
                "position" == a.property.name && "URLHash" == a.property.value && (a.data = this.hashes[b.location.hash.substring(1)]), this.filling = "item" == a.property.name && a.property.value && a.property.value.is(":empty")
            }, this)
        }, this.$element.on(this.handlers), void a(b).on("hashchange.owl.navigation", a.proxy(function() {
            var a = b.location.hash.substring(1),
                c = this.hashes[a] && this.hashes[a].index() || 0;
            return a ? (this.carousel.dom.oStage.scrollLeft = 0, void this.carousel.to(c)) : !1
        }, this))) : !1
    };
    c.Defaults = {
        URLhashListener: !1
